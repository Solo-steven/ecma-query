...
macro_rules! unexpect_token_error {
    ($self: expr, $token: expr) => {
      return Err(format!("[Error]: Got Unexpect Token {:?}, expect token {:?}", $self.get_token(), $token));   
    };
}
    ...
    fn parse_instruction(&mut self) -> ParseResult<InstructionNode<'a>> {
        ...
        let inst =  match self.lexer.cur_value().as_ref() {
            "query" => {
                ...
                Ok(InstructionNode::Query(QueryActionNode { target_node: node }))
            }
            _ => panic!()
        };
        ...
        match self.get_token() {
            TokenKind::BoolLiteral => {
                ...
                match raw_value.as_ref() {
                    "true" => {
                        self.next_token();
                        Ok(Literal::Bool(BoolLiteral { raw_value , value: true  }))
                    }
                    "false" => {
                        self.next_token();
                        Ok(Literal::Bool(BoolLiteral { raw_value , value: false  }))
                    }
                    _ => unreachable!()
                }
            }
